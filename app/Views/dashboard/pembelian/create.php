    ...
    <div class="container-fluid">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="<?= base_url('pembelian/store'); ?>" method="post" autocomplete="off">
                        <?= csrf_field(); ?>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="id_bahan" id="id_bahan" class="form-control form-select" required>
                                ...
                                <?php foreach ($bahan as $row) : ?>
                                    <option data-price="<?= $row['harga'] ?>" value="<?= $row['id_bahan'] ?>"><?= $row['nama'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="mb-3">
                            <label for="harga" class="form-label">Harga Bahan</label>
                            <input type="number" class="form-control" placeholder="Masukan harga Bahan" name="harga" id="harga" required min="1000" step="1000" oninvalid="this.setCustomValidity('Wajib diisi dengan kelipatan 1000')" oninput="this.setCustomValidity('')" readonly>
                        </div>
                        <div class="mb-3">
                            <label for="jumlah" class="form-label">Jumlah Bahan</label>
                            <input type="number" class="form-control" id="jumlah" name="jumlah" required min="1" oninvalid="this.setCustomValidity('Wajib diisi min 1 bahan')" oninput="this.setCustomValidity('')" value="0">
                        </div>
                        <div class="mb-3">
                            <label for="total" class="form-label">Total Biaya</label>
                            <input type="number" class="form-control" id="total" value="0" name="total" required min="1000" step="1000" oninvalid="this.setCustomValidity('Wajib diisi dengan kelipatan 1000')" oninput="this.setCustomValidity('')" readonly>
                        </div>
